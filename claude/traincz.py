...
def apply_interpolation_augmentation(data, methods=['linear', 'cubic', 'quadratic'], 
                                   augment_ratio=0.5, noise_level=0.01):
    """
    使用多种插值方法对训练数据进行增强
    
    Parameters:
    - data: 原始训练数据 (n_samples, n_features)
    - methods: 插值方法列表
    - augment_ratio: 增强数据的比例
    - noise_level: 添加的噪声水平
    """
    print(f"Applying interpolation augmentation with methods: {methods}")
    ...
    for method in methods:
        ...
        # 对每个特征分别进行插值
        for feature_idx in tqdm(range(data.shape[1]), desc=f"Processing {method} interpolation"):
            feature_data = data[:, feature_idx]
            
            # 创建原始时间索引
            original_indices = np.arange(len(feature_data))
            
            # 生成新的插值点
            n_new_points = int(original_length * augment_ratio)
            
            if method == 'linear':
                # 线性插值
                f = interpolate.interp1d(original_indices, feature_data, 
                                       kind='linear', fill_value='extrapolate')
                new_indices = np.linspace(0, len(feature_data)-1, n_new_points)
            ...
            # 添加小量噪声以增加多样性
            if noise_level > 0:
                noise = np.random.normal(0, noise_level * np.std(feature_data), 
                                       len(interpolated_values))
                interpolated_values += noise
            
            if feature_idx == 0:
                method_data = interpolated_values.reshape(-1, 1)
            else:
                method_data = np.hstack([method_data, interpolated_values.reshape(-1, 1)])
        
        augmented_data.append(method_data)
    
    # 合并所有增强数据
    final_data = np.vstack(augmented_data)
    print(f"Original data shape: {data.shape}")
    print(f"Augmented data shape: {final_data.shape}")
    print(f"Data augmentation ratio: {final_data.shape[0] / data.shape[0]:.2f}x")
    
    return final_data

def temporal_interpolation_augmentation(data, window_size=5, stride=2):
    """
    基于时间窗口的插值增强
    在相邻时间窗口之间生成插值数据
    """
    print("Applying temporal interpolation augmentation...")
    augmented_data = [data]  # 包含原始数据
    
    for i in tqdm(range(0, len(data) - window_size, stride), desc="Temporal interpolation"):
        if i + window_size * 2 < len(data):
            # 获取两个相邻窗口
            window1 = data[i:i + window_size]
            window2 = data[i + window_size:i + window_size * 2]
            
            # 在两个窗口之间生成插值数据
            interpolated_window = []
            for j in range(window_size):
                # 线性插值
                alpha = (j + 1) / (window_size + 1)
                interpolated_point = (1 - alpha) * window1[j] + alpha * window2[j]
                interpolated_window.append(interpolated_point)
            
            augmented_data.append(np.array(interpolated_window))
    
    final_data = np.vstack(augmented_data)
    print(f"Temporal augmentation - Original: {data.shape}, Final: {final_data.shape}")
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save({
                ...
                'epoch': epoch
            }, 'best_optimized_model_with_interpolation.pth')
        else:
            patience_counter += 1
...
if __name__ == "__main__":
    ...
    # 限制处理的列数以避免内存问题
    max_columns = min(1000, len(numeric_columns))
    ...
    print(f"Original train data shape: {train_data_raw.shape}")
    ...
    # 应用多种插值方法进行数据增强
    train_data_augmented = apply_interpolation_augmentation(
        train_data_raw, 
        methods=['linear', 'cubic'], 
        augment_ratio=0.3,  # 每种方法增加30%的数据
        noise_level=0.005   # 添加0.5%的噪声
    )
    
    # 应用时间插值增强
    train_data_final = temporal_interpolation_augmentation(
        train_data_augmented, 
        window_size=5, 
        stride=3
    )
    
    print(f"Final augmented train data shape: {train_data_final.shape}")
    
    # 4. 数据标准化
    print("Normalizing data...")
    train_data_norm, scalers = robust_normalize_data(train_data_final)
    
    # 对验证集和测试集使用相同的scaler（基于原始训练数据）
    ...
    train_losses, val_losses = train_model_with_validation(
        ...
        epochs=5000, batch_size=64, device=device
    )
    ...
    plt.subplot(2, 3, 2)
    sample_feature = 0
    plt.plot(train_data_raw[:200, sample_feature], label='Original', alpha=0.7)
    plt.plot(train_data_final[:200, sample_feature], label='Augmented', alpha=0.7)
    plt.title(f'Data Augmentation Effect (Feature {sample_feature})')
    ...
    # 9. 加载最佳模型并测试
    checkpoint = torch.load('best_optimized_model_with_interpolation.pth')
    ...
    print("\n=== Overall Metrics (With Data Augmentation) ===")
    ...
    # plt.subplot(2, 3, 3)
    # point_idx = 0
    # plt.plot(target_denorm[:50, point_idx], label='True', alpha=0.8, linewidth=2)
    # plt.plot(pred_denorm[:50, point_idx], label='Predicted', alpha=0.8, linewidth=2)
    ...
    # # 散点图
    # plt.subplot(2, 3, 4)
    # sample_indices = np.random.choice(len(target_denorm.flatten()), 5000, replace=False)
    # plt.scatter(target_denorm.flatten()[sample_indices], 
    #            pred_denorm.flatten()[sample_indices], alpha=0.5)
    # plt.plot([target_denorm.flatten().min(), target_denorm.flatten().max()], 
    #          [target_denorm.flatten().min(), target_denorm.flatten().max()], 'r--')
    ...
    # # 误差分布
    # plt.subplot(2, 3, 5)
    # errors = target_denorm.flatten() - pred_denorm.flatten()
    ...
    #     plt.plot(target_denorm[:30, i], label=f'True Point {i}', alpha=0.7)
    #     plt.plot(pred_denorm[:30, i], label=f'Pred Point {i}', alpha=0.7)