...
# extra_channels = np.zeros((4, image_data.shape[1], image_data.shape[2]))  # 创建 4 个全零通道
# image_data_with_extra_channels = np.concatenate((image_data, extra_channels), axis=0)

# test_images = torch.tensor(image_data_with_extra_channels).float().unsqueeze(0)
...
# 将图像数据转换为 NumPy 数组
image_data_with_extra_channels = image_data_with_extra_channels.transpose(1, 2, 0)  # 转换为 [H, W, C] 格式
image_data_resized = cv2.resize(image_data_with_extra_channels, (256, 256))  # 重新调整图像大小

# 将调整大小后的数据转换回原始格式并转换为 PyTorch 张量
image_data_resized = image_data_resized.transpose(2, 0, 1)  # 转换回 [C, H, W] 格式
test_images = torch.tensor(image_data_resized).float().unsqueeze(0)

# 模型预测
outputs = model(test_images)

# 将预测结果二值化
predicted_mask = (outputs > 0.7).float().squeeze().detach().numpy()
...
# 创建一个三通道的彩色掩码，红色通道显示预测掩码
colored_mask = np.zeros_like(original_image)
colored_mask[:, :, 2] = predicted_mask_resized   # 将掩码设置为红色
