...
def save_predictions_to_csv(predictions, targets, filename, num_points_to_save=1000, max_sequences=100):
    """
    将预测结果保存为CSV文件
    :param predictions: 预测值数组 (num_sequences, pre_len, num_points)
    :param targets: 真实值数组 (num_sequences, pre_len, num_points)
    :param filename: 输出文件名
    :param num_points_to_save: 要保存的点数量
    :param max_sequences: 要保存的最大序列数量
    """
    num_sequences = min(max_sequences, predictions.shape[0])
    num_points = min(num_points_to_save, predictions.shape[2])
    ...
    for seq_idx in range(num_sequences):
        for timestep in range(predictions.shape[1]):
            row = {
                'sequence_id': seq_idx,
                'timestep': timestep + 1  # 时间步从1开始
            }
            
            # 添加每个点的预测值和真实值
            for point_idx in range(num_points):
                pred = predictions[seq_idx, timestep, point_idx]
                true = targets[seq_idx, timestep, point_idx]
                row[f'point_{point_idx}_pred'] = pred
                row[f'point_{point_idx}_true'] = true
    ...
    print(f"预测结果已保存到 {filename} (序列: {num_sequences}, 点: {num_points})")
...
if __name__ == "__main__":
    ...
    # 6. 保存top 10点的预测结果到CSV
    print("Saving predictions for top points...")
    top_points_indices = [p['point_index'] for p in top_10_points]
    for rank, point in enumerate(top_10_points):
        point_idx = point['point_index']
        
        # 提取该点的预测结果和真实值
        point_pred = all_pred_denorm[:, :, point_idx]
        point_target = all_target_denorm[:, :, point_idx]
        
        # 创建DataFrame
        data = []
        for seq_idx in range(point_pred.shape[0]):
            for timestep in range(point_pred.shape[1]):
                data.append({
                    'sequence_id': seq_idx,
                    'timestep': timestep + 1,
                    'predicted': point_pred[seq_idx, timestep],
                    'true': point_target[seq_idx, timestep]
                })
        
        df = pd.DataFrame(data)
        df.to_csv(f'top_point_{point_idx}_rank_{rank+1}_predictions.csv', index=False)
        print(f"Saved predictions for point {point_idx} (rank {rank+1}) to top_point_{point_idx}_rank_{rank+1}_predictions.csv")
